...
export class RoleManagementService {
  ...
  async syncRole(
    ...
    // メンバーをサーバーから取得
    const usersToAdd = await guild.members.fetch({
      user: [...usersToAddIdSet],
    });
    // すでにロールを持っているユーザーを取得
   ...
   * @returns ロール同期結果
   */
  async syncRoleByCondition(guild: Guild): Promise<string> {
    ...
    for (const [roleId, count] of Object.entries(
      ...
      // ロールを同期
      const result = await this.syncRole(guild, roleId, userIds);
      if (result) {
        resultText += `\`\`\`\n${result}\`\`\`\n`;
      }
    }
    ...
    const result = await this.syncRole(
      guild,
      config.event_join_role_id,
      userIds,
    );
    if (result) {
      resultText += `\`\`\`\n${result}\`\`\`\n`;
