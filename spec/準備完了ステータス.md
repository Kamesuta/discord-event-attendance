...
`src/commands/eventCreatorCommand/EventCreatorScheduleCommand.ts` を修正し、以下の機能を追加する。
...
- **新しい準備状況パネルの作成と送信**:
    ...
    - `src/messageUpdaters/` に `PreparationStatusMessageUpdater` クラスを追加し、これを使用してパネルを表示する。
...
3.  `src/commands/eventCreatorCommand/EventCreatorScheduleCommand.ts` を修正し、`show` パラメータによる制御、既存パネルの削除と新しいパネルの作成・送信ロジックを実装する。（`config.event_panel_channel_id` を使用）
