...
`src/commands/eventCreatorCommand/EventCreatorCreateCommand.ts` を修正:
...
## 5. `eventHandler.ts` の修正

`src/eventHandler.ts` の `onCreateScheduledEvent` と `onStartScheduledEvent`:
...
5. `eventHandler.ts` の `onCreateScheduledEvent` を修正
...
6. `eventHandler.ts` の `onStartScheduledEvent` も同様に修正
